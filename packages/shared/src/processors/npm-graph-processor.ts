...
export class NpmGraphProcessor implements Processor {
    ...
    async run(ctx: ExecutionContext, session: AnalysisSession): Promise<void> {
        const { target, requested } = session.meta;
        ...
        await this.resolveDependencies(ctx, graph, rootNode);
    }
    ...
    private async resolveDependencies(
        ...
        const deps = (node.manifest as NpmManifest).dependencies ?? {};
        ...
        for (const [name, range] of Object.entries(deps)) {
            ...
            if (!graph.hasNode(id)) {
                ...
                await this.resolveDependencies(ctx, graph, child);
            } else {
                graph.addEdge(node.id, id);
            }
        }
